...
tree.on("nodeselect", function (e) {
    if (e.node.dicPath) {
        var url = "/info/ggdiccontentlist?path=" + e.node.dicPath;
...
function add() {
    ShowCommonDialog("/info/ggdicedit", "广告位字典信息 - 新增", 470, 300);
}
...
function edit() {
    ...
    if (row) {
        if (row.id > 0)
            ShowCommonDialog("/info/ggdicedit?id=" + row.id, "广告位字典信息 - 编辑", 470, 300);
        else
